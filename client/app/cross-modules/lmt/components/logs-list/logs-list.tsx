...
const NewDataAvailableIndicator = ({
  onBottomClick,
}: {
  onBottomClick: () => void;
}) => (
...
export const LogsList = () => {
  ...
  const { initialLogs, isLoading, hasTopMore, fetchOldLogs, fetchNewLogs } =
    useLogs({
      serviceName,
      serviceNames,
      search: search,
  ...
  const fetchNewLogsHandler = async (
    lastItemTimestamp: string = initialTimeStamp,
  ) => {
      ...
      <CardContent className="h-[calc(100vh-340px)]">
        {isManagedLoading ? (
          ...
          <InfiniteScroll<ILog>
            ...
            bottomIndicator={(cb) => (
              <NewDataAvailableIndicator onBottomClick={cb} />
            )}
          />
        )}
      </CardContent>
