...
export const ConfigureCaptchaList = ({ isLoading, configuration }: ConfigureCaptchaListProps) => {
  ...
  return (
    <Card className={cn("transition-colors", enabled ? "border-border" : "border-border")}>
      ...
      <CardContent className="border-t pt-4">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Item label="Secret Key">
            {/* The backend never returns the secret itself — only whether one is set. */}
            <span className="text-muted-foreground">
              {configuration.secretId ? "Configured" : "Not set"}
            </span>
          </Item>
        </div>
      </CardContent>
