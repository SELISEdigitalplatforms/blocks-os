...
vi.mock("@blocks-idp/settings/components/organization-config-form", () => ({
  OrganizationConfigForm: ({ config }: { config: unknown }) => <div data-testid="form">{JSON.stringify(config)}</div>,
}));
...
vi.mock("@blocks-idp/settings/components/settings-tab-loading-state", () => ({
  OrganizationTabLoadingState: () => <div data-testid="loading" />,
}));
...
import { OrganizationConfigTab } from "./organization-config-tab";

describe("OrganizationConfigTab", () => {
  beforeEach(() => {
    vi.clearAllMocks();
