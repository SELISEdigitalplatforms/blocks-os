...
function mapApiResponseToEndpoint(data: Record<string, unknown>): IApiEndpoint {
  ...
  return {
    ...
    lastUpdatedDate: getProperty<string>(
      "lastUpdatedDate",
      "LastUpdatedDate",
      "",
    ),
    ...
    organizationIds: getProperty<string[]>(
      "organizationIds",
      "OrganizationIds",
      [],
    ),
    ...
    isCaptchaRequired: getProperty<boolean>(
      "isCaptchaRequired",
      "IsCaptchaRequired",
      false,
    ),
    captchaProvider: getProperty<string>(
      "captchaProvider",
      "CaptchaProvider",
      "",
    ),
    isMFARequired: getProperty<boolean>(
      "isMFARequired",
      "IsMFARequired",
      false,
    ),
...
class ApiSettingsService {
  getEndpoints(
    payload: IGetApiEndpointsPayload,
  ): Promise<IGetApiEndpointsResponse> {
    return http
      .post(API_SETTINGS_ENDPOINTS.GET_LIST, {
        page: payload.page ?? 0,
      ...
      .then((response: unknown) => {
        ...
        return {
          page:
            ((typedResponse.page as number) ||
              (typedResponse.Page as number)) ??
            0,
          pageSize:
            ((typedResponse.pageSize as number) ||
              (typedResponse.PageSize as number)) ??
            100,
          totalPages:
            ((typedResponse.totalPages as number) ||
              (typedResponse.TotalPages as number)) ??
            0,
          totalCount:
            ((typedResponse.totalCount as number) ||
              (typedResponse.TotalCount as number)) ??
            0,
          data: data.map(mapApiResponseToEndpoint),
          errors:
            (typedResponse.errors as string[] | null) ||
            (typedResponse.Errors as string[] | null),
  ...
  updateEndpoint(
    payload: IUpdateApiEndpointPayload,
  ): Promise<IUpdateApiEndpointResponse> {
    return http.post(API_SETTINGS_ENDPOINTS.UPDATE, payload);
  }

  bulkUpdate(
    payload: IBulkUpdateApiEndpointsPayload,
  ): Promise<IBulkUpdateApiEndpointsResponse> {
    return http.post(API_SETTINGS_ENDPOINTS.BULK_UPDATE, payload);
  }

  removeEndpoints(
    payload: IRemoveApiEndpointsPayload,
  ): Promise<IRemoveApiEndpointsResponse> {
