    ...
    return http
      ...
      .then((response: unknown) => {
        ...
        return {
          ...
          pageSize:
            ((typedResponse.pageSize as number) || (typedResponse.PageSize as number)) ?? 100,
          totalPages:
            ((typedResponse.totalPages as number) || (typedResponse.TotalPages as number)) ?? 0,
          totalCount:
            ((typedResponse.totalCount as number) || (typedResponse.TotalCount as number)) ?? 0,
          data: data.map(mapApiResponseToEndpoint),
          errors:
            (typedResponse.errors as string[] | null) || (typedResponse.Errors as string[] | null),
