...
namespace XUnitTest.Integration
{
    ...
    public class CaptchaConfigServiceIntegrationTests
    {
        ...
        public async Task SaveGetDelete_RoundTripsThroughRealMongoViaGenesissKeyValueStore()
        {
            ...
            blocksSecret.SetupGet(s => s.DatabaseConnectionString).Returns(MongoIntegrationFixture.ConnectionString);
            var store = new MongoKeyValueStore(_fixture.DbContextProvider, blocksSecret.Object);
            ...
            // Nothing saved yet under a second, unrelated key.
            (await service.GetAsync(Guid.NewGuid().ToString("N"))).Should().BeNull();

            var afterCreate = await service.GetAsync(created.Id);
            ...
            // A second, independent record — must not disturb the first.
            var secondCreated = await service.SaveAsync(new SaveCaptchaConfigRequest
            {
                IsEnable = true,
                Provider = "hcaptcha",
                CaptchaKey = "site-key-other",
                CaptchaGenerator = "EasyCaptchaGenerator"
            });
            secondCreated.Id.Should().NotBe(created.Id);

            var list = await service.GetListAsync();
            list.Select(r => r.Id).Should().Contain(new[] { created.Id, secondCreated.Id });

            // Update the first record, this time adding a secret — should call SetAsync (create), not Rotate.
            ...
            var afterUpdate = await service.GetAsync(created.Id);
            afterUpdate!.Provider.Should().Be("hcaptcha");
            afterUpdate.IsEnable.Should().BeFalse();
            afterUpdate.SecretId.Should().Be("sec-it-1");

            // The second record must be untouched by the first's update.
            (await service.GetAsync(secondCreated.Id))!.Provider.Should().Be("hcaptcha");
            (await service.GetAsync(secondCreated.Id))!.SecretId.Should().BeNull();
            ...
            await service.DeleteAsync(secondCreated.Id);
        }
    }
}
