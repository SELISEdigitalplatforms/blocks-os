 ...
 * .NET host bakes `BLOCKS_OS_BASE_URL` from the Mongo `blocks-secret-os` secret,
 * which is the deployed host WITHOUT a port (https://dev-os.blocksdevelopers.com).
 * When we run OS locally on :5000, the SPA would then send its API calls (e.g.
 * /api/Project/Gets) to the remote dev server, so the console shows no local
 * data. This patches the served index.html so BLOCKS_OS_BASE_URL === E2E_BASE_URL.
 *
 * Idempotent and order-independent: it rewrites the concrete value (or the
 * `__BLOCKS_OS_BASE_URL__` placeholder), so it holds whether it runs before or
...
export default function globalSetup() {
  ...
  console.log(`[e2e] Patched BLOCKS_OS_BASE_URL -> "${baseURL}" in served index.html.`);
}
